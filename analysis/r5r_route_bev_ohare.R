...
ev <- data.frame(id = "beverly-95th", lon = -87.6672222, lat = 41.7213889)
pi <- data.frame(id = "ohare-terminals", lon = -87.904444, lat = 41.977800)
...
route_on <- function(folder, label) {
  ...
  fwrite(dt[, .(network, option, departure_time, total_duration, segment,
                mode, route, segment_duration, wait)],
         file.path(sp, paste0("bev_ohare_", label, ".csv")))
