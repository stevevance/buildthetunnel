...
o <- data.frame(id="roseland", lon=-87.6205, lat=41.6996)   # ~107th & Michigan
d <- data.frame(id="rosemont", lon=-87.85955, lat=41.98363) # Rosemont Blue Line
for (f in c("r5r_data","r5r_data_rle","r5r_data_xr_rle")) {
  ...
  dt <- as.data.table(it)
  lab <- switch(f, r5r_data="today", r5r_data_rle="rle", r5r_data_xr_rle="rle_ct")
  fwrite(dt[, .(option, departure_time, total_duration, segment, mode, route, segment_duration, wait)],
         file.path(sp, paste0("roseland_", lab, ".csv")))
